<?php
//Joseph Thayer
//Move task from late table to completed table
include "connectAndCheck.php"; //pass pdo variable and check if any tasks are late
 

try{
    // start query
	$query = "INSERT INTO complete (task_id, task_name, due_date) SELECT task_id, task_name, due_date FROM late WHERE task_id=:task_id"; 
	$stmt = $pdo->prepare($query);
	$stmt->bindParam(':task_id', $_REQUEST['task_id']);
	if($stmt->execute())
	{
    $sql = "DELETE FROM late WHERE task_id=:task_id";
